...
describe('EventEmitterModule - e2e', () => {
  ...
  it('should be able to gracefully recover when an unexpected error occurs from durable request scoped', async () => {
    await app.init();

    const eventEmitter = app.get(EventEmitter2);
    const result = eventEmitter.emit('error-handling.durable-request-scoped');

    expect(result).toBeTruthy();
  });

  it('should be able to gracefully recover when an unexpected error occurs from durable request scoped and suppressErrors is true', async () => {
    await app.init();

    const eventEmitter = app.get(EventEmitter2);
    const result = eventEmitter.emit(
      'error-handling-suppressed.durable-request-scoped',
    );
